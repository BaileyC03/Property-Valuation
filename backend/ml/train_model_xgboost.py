#!/usr/bin/env python3
"""
Train XGBoost model using HM Land Registry real data.
Expected accuracy: MAE ~£25-40k (vs Keras FCNN ~£50-70k)
...
def train_model():
    """Train XGBoost model with HM Land Registry data."""
    print("\n" + "="*70)
    print("Training XGBoost Model with HM Land Registry Data")
    print("="*70)
    ...
    # Scale features (helps XGBoost learn better)
    print("\nScaling features...")
    ...
    print("(This will take 5-10 minutes for 574k samples)")
    ...
    print(f"\n📊 MODEL PERFORMANCE (XGBoost with Land Registry data):")
    ...
    print(f"  XGBoost MAE:    ±£{test_mae:,.0f}")
    if test_mae < 50000:
    ...
    model.save_model('ml/model_xgboost.json')
    joblib.dump(model, 'ml/model_xgboost.joblib')
    joblib.dump(scaler, 'ml/scaler_xgboost.joblib')

    print(f"  ✓ Model saved to ml/model_xgboost.joblib")
    print(f"  ✓ Scaler saved to ml/scaler_xgboost.joblib")

    print(f"\n✅ XGBoost training complete!")
    print(f"\nNext steps to use this model:")
    print(f"  1. Update backend/app.py to load XGBoost model")
    print(f"  2. Set model priority: XGBoost → LightGBM → Keras → RandomForest")
